...
import { ArrowLeft, User, Building, Briefcase, Clock } from "lucide-vue-next";
...
async function loadProfile() {
  try {
    ...
    if (!name) {
      errorMsg.value = "缺少用户名参数";
      return;
    }
    profile.value = await api(`/api/users/by-name?name=${encodeURIComponent(name)}`);
  } catch (e) {
    errorMsg.value = e.message || "无法加载用户资料";
...
<template>
  ...
  <div v-else-if="errorMsg" class="empty-state" style="padding:60px 0;">{{ errorMsg }}</div>
